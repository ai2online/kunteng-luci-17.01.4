...
function wifi_setup()
	...
	if set_wireless_iface(cfg2g,
			reqData.wifi0_ssid,
			reqData.wifi0_encryption,
			reqData.wifi0_password,
			reqData.wifi0_hidden,
			reqData.wifi0_disabled) == 1 then
		respCode = 1
	end
	...
	if cfg5g ~= nil then
		if reqData.merge == 1 then
			...
			if set_wireless_iface(cfg5g,
					reqData.wifi1_ssid,
					reqData.wifi1_encryption,
					reqData.wifi1_password,
					reqData.wifi1_hidden,
					reqData.wifi1_disabled) == 1 then
				respCode = 1
			end
		end
	end
	...
	if reqData.wifi0_guest_ssid then
		...
		if set_wireless_iface(cfg,
				reqData.wifi0_guest_ssid,
				reqData.wifi0_guest_encryption,
				reqData.wifi0_guest_password,
				reqData.wifi0_guest_hidden,
				reqData.wifi0_guest_disabled) == 1 then
			respCode = 1
		end
	end
	...
	ktUtil.fork_exec("sleep 1;wifi reload;sleep 1;ifup wan")
end
...
function qca_set_apclient(section)
	...
	ktUtil.fork_exec("/usr/sbin/qcawifi-sta-daemon.sh")
	return true
end
