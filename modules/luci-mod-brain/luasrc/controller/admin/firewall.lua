...
function _setPortForward(name, ip, sport, dport, proto)
	if ip and _portCheck(tonumber(sport)) and _portCheck(tonumber(dport)) then
		if _portForwardInfo().status == 2 then
			return 3
		end
		
		if _portConflictCheck(sport) ~= nil then
			return 2
		end
